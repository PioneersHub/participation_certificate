...
from src import all_fonts, logger
...
class Certificates:
    ...
    def __init__(self, attendees: list[Attendee],
        ...
        self.save_to = Path(__file__).parents[1] / "_certificates" / self.event
    ...
    def generate_certificate(self, attendee):
        ...
        fpdf.add_font("poppins-regular", "I", font.parents[1] / "Poppins/Poppins-Italic.ttf")
    ...
    def save(self, attendee: Attendee, fpdf):
        save_to = self.save_to / f"{attendee.uuid}" / quote_plus(f"{self.event}-certificate-{attendee.uuid}.pdf")
